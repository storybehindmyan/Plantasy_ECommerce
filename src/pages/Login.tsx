...
const Login = ({ initialView = "login" }: LoginProps) => {
  const [view, setView] = useState<
    "signup-options" | "email-signup" | "login"
  >(initialView);
  ...
  const handleGoogleClick = async () => {
    setLocalError(null);
    try {
      await loginWithGoogle(); // popup handled in hook
    } catch (err: any) {
        ...
        {view === "signup-options" && (
          <div className="w-full flex flex-col gap-4">
            ...
            <button
              type="button"
              onClick={() =>
                setLocalError("Facebook auth not implemented yet.")
              }
            ...
            {view === "login" && (
              <div className="w-full flex flex-col gap-4 mb-6">
                ...
                >
                  <GoogleIcon />
                  <span>
                    {isSubmitting ? "Please wait..." : "Log in with Google"}
                  </span>
                </button>
