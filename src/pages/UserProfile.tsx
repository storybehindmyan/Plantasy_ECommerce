...
import { Invoice } from '../hooks/Invoice';
...
const MyOrders: React.FC = () => {
  ...
  // ✅ UPDATED: use Invoice service to download invoice
  const handleGenerateInvoice = async (order: OrderDoc) => {
    try {
      await Invoice.download(order.id); // uses orders/{id} to build & download PDF
    } catch (e) {
...
const Addresses: React.FC = () => {
  ...
  // track address being edited (null = new address)
  ...
  const handleSave = async () => {
    ...
    if (isSubmitting) {
      alert(
        "Your address is already being saved. Please wait and avoid clicking multiple times."
      );
      return;
    }
    ...
    try {
      ...
      // only set createdAt for new addresses
      if (!editingAddressId) {
        baseData.createdAt = serverTimestamp();
      }

      await setDoc(addrRef, baseData, { merge: true });
        ...
        <>
          <div className="flex-1 flex flex-col items-center justify-center mb-12">
            {loading ? (
              ...
              <div className="w-full grid grid-cols-1 md-grid-cols-2 gap-4 md:grid-cols-2">
                {addresses.map((addr) => (
                  <div
                    key={addr.id}
